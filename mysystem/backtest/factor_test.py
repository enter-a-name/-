...
def wrapup_test(pctdf,data,agg_func,require_returns = False, require_submit = False, cta = False):
    ...
    if not cta:
        factor = factor_constructor.get_independent_factor(data,agg_func)
...
def plot(results):
    ...
    plt.plot((clean_results+1).cumprod())
...
def get_corr(results):
    ...
    for i in candidates:
        ...
        new_ret = pd.DataFrame(results.iloc[:,0] - results.iloc[:,-1]).reset_index().rename(columns={0:'new'})
